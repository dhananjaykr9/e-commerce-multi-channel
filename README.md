...
├── scripts/
│   ├── local_pyspark_transform.py        # PySpark distributed ETL script (Airflow task)
│   └── load_snowflake.py                 # Snowflake dimensional warehouse loader (Airflow task)
├── src/
│   ├── database/
│   │   ├── mysql_schema.sql              # MySQL operational database DDL
│   │   ├── mysql_seed.sql                # Initial transactional seed data
│   │   └── snowflake_schema.sql          # Snowflake dimensional star schema DDL (Worksheet)
│   ├── glue/
...
2. **Create AWS S3 Bucket (AWS Console)**:
   - Log in to the **AWS Management Console** > **Amazon S3**.
   - Create bucket: `e-commerce-multi-channel-dhananjay99` (in region `ap-south-1`).
   - The pipeline will automatically create partitions (`raw-landing/`, `curated/`, `bad-records/`) upon ingestion.

3. **Deploy Snowflake Star Schema (Snowflake Web UI)**:
   - Log in to **Snowflake Web UI** > **Worksheets**.
   - Open and execute the SQL statements in [`src/database/snowflake_schema.sql`](src/database/snowflake_schema.sql) to create the warehouse, database, schema, dimension tables, fact table, and stage.
